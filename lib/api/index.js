...
class RTLSDR extends EventEmitter {
	...
	tunerGain(gain, exact) {
		...
		if (typeof ga !== 'undefined') {
			...
			if (ga === 'auto') {
				librtlsdr.set_tuner_ga_mode(this.device, 0);
				this.lastGainMode = 0;
			} else {
				if (typeof ga === 'number') {
					if (!ex) {
	 ...
	 * @see {@link https://github.com/steve-m/librtlsdr/blob/8b4d755ba1b889510fba30f627ee08736203070d/include/rtl-sdr.h#L276 rtlsdr_setlastTestmode}
	...
	testmode(on) {
		...
		if (typeof on !== 'undefined') {
			librtlsdr.setlastTestmode(this.device, on);
	...
	agc(on) {
		...
		if (typeof on !== 'undefined') {
			librtlsdr.setlastAGC_mode(this.device, on);
			this.lastAGC = on;
			return this;
		}
